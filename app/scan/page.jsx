...
export default function ScanPage() {
  ...
  const handleScan = async (scannedToken) => {
    ...
    try {
      ...
      if (updateErr) console.warn('Scan record warning:', updateErr.message);
      ...
      <div className="scan-main">
        ...
        {error && (
          <div className="scan-error-card">
            <X size={24} />
            <p>{error}</p>
            <button onClick={() => { setError(''); setToken(''); if (inputRef.current) inputRef.current.focus(); }}>Dismiss</button>
          </div>
        )}
