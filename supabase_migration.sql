...
-- RPC: Count scans per event for reports
CREATE OR REPLACE FUNCTION get_event_scan_counts()
RETURNS TABLE(event_id UUID, count BIGINT) AS $$
BEGIN
  RETURN QUERY
  SELECT e.id, COUNT(es.id)::BIGINT
  FROM scan_events e
  LEFT JOIN event_scans es ON es.event_id = e.id
  GROUP BY e.id;
END;
$$ LANGUAGE plpgsql;
...
-- Create upcoming_events table for homepage event schedule
...
-- Allow public read access to active upcoming events
CREATE POLICY IF NOT EXISTS "Allow public select active upcoming events"
  ON upcoming_events
  FOR SELECT
  TO anon
  USING (status = 'Active');

-- Allow authenticated users full CRUD on upcoming_events
CREATE POLICY IF NOT EXISTS "Allow authenticated CRUD upcoming events"
  ON upcoming_events
  FOR ALL
  TO authenticated
  USING (true)
...
-- Create storage bucket for event images (idempotent)
-- NOTE: Run this in your Supabase SQL Editor
INSERT INTO storage.buckets (id, name, public)
VALUES ('event-images', 'event-images', true)
ON CONFLICT (id) DO NOTHING;

-- Allow public read access to event-images bucket
CREATE POLICY IF NOT EXISTS "Allow public read event images"
  ON storage.objects
  FOR SELECT
  TO anon
...
CREATE POLICY IF NOT EXISTS "Allow authenticated upload event images"
  ON storage.objects
  FOR INSERT
  TO authenticated
  WITH CHECK (bucket_id = 'event-images');

-- Allow authenticated users to delete from event-images bucket
CREATE POLICY IF NOT EXISTS "Allow authenticated delete event images"
  ON storage.objects
  FOR DELETE
  TO authenticated
  USING (bucket_id = 'event-images');
...
-- Contact form messages from homepage
...
CREATE POLICY IF NOT EXISTS "Allow public insert contact_messages"
  ON contact_messages FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow authenticated select contact_messages"
...
-- Add suffix column to existing tables (if not exists)
ALTER TABLE "ValidResidents" ADD COLUMN IF NOT EXISTS suffix TEXT;
ALTER TABLE registrations ADD COLUMN IF NOT EXISTS suffix TEXT;

-- Extract suffix from existing middle_name values
-- Common suffixes: JR, JR., SR, SR., III, IV, V, II
UPDATE "ValidResidents"
SET suffix = TRIM(middle_name),
    middle_name = NULL
WHERE middle_name IS NOT NULL
  AND TRIM(UPPER(REPLACE(middle_name, '.', ''))) IN ('JR', 'SR', 'III', 'IV', 'V', 'II', '2ND', '3RD');

-- Sync registrations.suffix from ValidResidents.suffix for existing records
...
CREATE INDEX IF NOT EXISTS idx_validresidents_suffix ON "ValidResidents" (suffix);
