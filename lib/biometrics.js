...
export function generatePerceptualFaceToken(source) {
  ...
  try {
    // If source is a string (Data URL or base64)
    if (typeof source === 'string') {
      if (typeof window === 'undefined') return null; // Server fallback
      const img = new Image();
      img.src = source;
      if (!img.complete) {
        return new Promise((resolve) => {
          img.onload = () => resolve(processImageSource(img));
          img.onerror = () => resolve(null);
        });
      }
      return processImageSource(img);
    }
...
function processImageSource(element) {
  ...
  // Convert 16x16 grid to grayscale luminance (256 pixels)
  ...
  // 256 bits = 64 hex characters (4 bits per hex character)
  // Part A (128 bits): Horizontal Gradient Differences (dHash)
  // Part B (128 bits): Luminance comparison against local average (aHash)
  const bits = [];

  // Part A: 16 rows * 8 gradient comparisons = 128 bits
  for (let y = 0; y < 16; y++) {
    for (let x = 0; x < 8; x++) {
      const idx1 = y * 16 + x * 2;
      const idx2 = y * 16 + x * 2 + 1;
      bits.push(luma[idx2] >= luma[idx1] ? 1 : 0);
    }
  }

  // Part B: 16 rows * 8 central pixels vs avgLuma = 128 bits
  for (let y = 0; y < 16; y++) {
    for (let x = 0; x < 8; x++) {
      const idx = y * 16 + (x + 4); // sample central region
      bits.push(luma[idx] >= avgLuma ? 1 : 0);
    }
  }
